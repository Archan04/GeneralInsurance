...
public class VehicleServiceImpl implements VehicleService {
	...
	public Vehicle register(Vehicle vehicle) {
		if (vehiclerepo.isVehiclePresent(vehicle.getRegNo()))
			throw new VehicleServiceException("Vehicle already registerd");
		Vehicle newVehicle = (Vehicle) vehiclerepo.store(vehicle);
		return newVehicle;
	...
	public List<Estimate> getPremiumPlans(Vehicle vehicle) {
		...
		VehicleModels vehicleModel = vehiclerepo.findPriceOfVehicle(vehicle.getManufacturer(), vehicle.getModel());
	...
	public VehicleModels addNewVehicle(VehicleModels vehicleModel) {
		if (vehiclerepo.isVehicleModelPresent(vehicleModel.getManufacturer(),vehicleModel.getModel()))
			throw new VehicleServiceException("Vehicle Model already exists!");		
		return (VehicleModels) vehiclerepo.store(vehicleModel);
	}

	@Override
	@Transactional
	public List<VehicleModels> listAvailableVehicle() {		
		return vehiclerepo.fetchAll(VehicleModels.class);
	}
